...
3) 원클래스 : Circle2 => Figure 클래스를 상속받아 작성하기
   멤버변수 :      반지름 (r),     Point p
   생성자(도형종류, r, x, y)  :   Point(x, y)
   메소드 
     도형의 넓이 area()) : Math.PI를 이용
     도형의 둘레(length()) : Math.PI를 이용
4)사각형 : Rectangle2 => Figure 클래스를 상속받아 작성하기
   멤버변수 ,   Point p1,   Point p2
...
abstract class Figure {
	...
	abstract double area();	abstract double length();}

class Circle2  extends Figure {
	int r;	Point p;
	Circle2(String type, int r, int x, int y) {
		super(type);		this.r=r;		p = new Point(x,y);	}
	@Override
	double area() {
...
class Rectangle2  extends Figure {
	Point p1;   Point p2;
	Rectangle2(String type, int x1, int y1, int x2, int y2) {
		super(type);	
		p1 = new Point(x1, y1) ;	
		p2 = new Point(x2, y2) ;}
	@Override
