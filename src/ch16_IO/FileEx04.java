...
class FileEx04 {
	static int totalFiles = 0;	static int totalDirs = 0;
	public static void main(String[] args) {
	    ...
	    System.out.println("총 "+ totalDirs  + "개의 디렉토리");	} // main
	public static void printFileList(File dir) {
	  ...
	  for(int i = 0 ; i < files.length ; i++) {
		  {  String filename = files[i].getName();
			  if (files[i].isDirectory()) {	  filename = "["+filename +"]";
				  subDir.add(i);  }
			  System.out.println(filename);		  } 	  	  }  
	  totalDirs +=subDir.size();  	  totalFiles +=(files.length - subDir.size());
	  System.out.println((files.length - subDir.size())+"개의 파일"
	  + subDir.size()+"개의 디렉토리");	 
